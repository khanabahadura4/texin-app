...
export const Header: React.FC<HeaderProps> = ({
  ...
  const navItems: Array<{ tab: 'feed' | 'factories' | 'alumni' | 'jobs'; icon: React.ReactNode; label: string }> = [
  ...
  return (
    <header className="sticky top-0 z-40 bg-[#005244] text-white shadow-md transition-colors border-b border-brand-800">
      <div className="max-w-[1500px] mx-auto px-3 sm:px-4">
          ...
          <nav className="hidden md:flex items-stretch">
            {navItems.map(item => (
              ...
              >
                ...
                {item.tab === 'factories' && (
                  <span className="absolute top-2 right-4 lg:right-6 w-2 h-2 bg-brand-300 rounded-full animate-ping" />
                )}
              </button>
            ))}
          </nav>

          {/* Right Controls: circular icon buttons like Facebook, then Sign Up + Avatar */}
          <div className="flex items-center justify-end space-x-1.5 sm:space-x-2 basis-0 flex-1">
            {/* Language Toggle */}
            <button
              onClick={toggleLanguage}
              className="w-9 h-9 rounded-full bg-brand-950/40 hover:bg-brand-900/60 text-[10px] font-bold text-white transition flex items-center justify-center"
            ...
            <button
              ...
              className={`w-9 h-9 rounded-full transition flex items-center justify-center ${
                activeTab === 'settings'
                  ? 'bg-white text-[#005244]'
                  : 'bg-brand-950/40 hover:bg-brand-900/60 text-white'
              }`}
            ...
            <button
              onClick={() => logout()}
              className="hidden sm:flex w-9 h-9 rounded-full bg-brand-950/40 hover:bg-red-600 text-white transition items-center justify-center"
            ...
            <div className="relative">
              ...
              {showProfileMenu && (
                <div className="absolute right-0 mt-2 w-72 bg-white dark:bg-slate-900 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-800 p-3 z-50 space-y-3 text-slate-900 dark:text-slate-100">
                  ...
                  {/* Mobile-only primary nav inside the dropdown, since the center nav is hidden below md */}
                  <div className="md:hidden border-t border-slate-100 dark:border-slate-800 pt-2 space-y-0.5">
                    {navItems.map(item => (
                      <button
                        key={item.tab}
                        onClick={() => {
                        ...
                        className={`w-full flex items-center gap-2 text-left p-2 rounded-lg text-xs font-semibold transition ${
                          activeTab === item.tab
                            ? 'bg-brand-50 dark:bg-brand-950/40 text-[#005244] dark:text-brand-400'
        ...
        {/* Mobile bottom tab row (icon-only, matches the desktop center nav) */}
        <nav className="md:hidden flex items-stretch justify-around border-t border-brand-800/60">
          {navItems.map(item => (
            <button
              key={item.tab}
              onClick={() => setActiveTab(item.tab)}
              title={item.label}
              className={`relative flex-1 flex items-center justify-center h-11 transition ${
