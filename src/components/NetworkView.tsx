...
export const NetworkView: React.FC<NetworkViewProps> = ({ onSelectUser, onOpenMessage }) => {
  ...
  // Every friend action goes through this wrapper so a Firestore permission error
  // (or any other failure) shows up on screen instead of the button silently doing nothing.
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-4">
      ...
      {/* Error banner — surfaces Firestore permission errors etc. instead of a silent no-op */}
      {errorMsg && (
        <div className="flex items-start gap-2 bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-900 text-red-700 dark:text-red-400 text-xs rounded-xl p-3">
          ...
          {suggestions.length === 0 ? (
            ...
            suggestions.map(u => (
              <UserRow key={u.id} user={u}>
                <button
                  onClick={() => runAction(u.id, () => sendFriendRequest(u.id))}
                  disabled={pendingActionId === u.id}
                  className="px-3 py-1.5 bg-[#005244] hover:bg-[#002e26] disabled:opacity-50 text-white font-bold text-xs rounded-lg shadow-sm transition flex items-center gap-1.5"
                >
            ...
            incomingRequests.map(r => {
              ...
              return (
                <UserRow key={r.id} user={u}>
                  <button
                    onClick={() => runAction(r.id, () => acceptFriendRequest(r.id))}
                    disabled={pendingActionId === r.id}
                    title={language === 'BN' ? 'গ্রহণ করুন' : 'Accept'}
                    className="p-2 bg-[#005244] hover:bg-[#002e26] disabled:opacity-50 text-white rounded-lg shadow-sm transition"
                  >
                    <Check className="w-4 h-4" />
                  </button>
                  <button
                    onClick={() => runAction(r.id, () => declineFriendRequest(r.id))}
                    disabled={pendingActionId === r.id}
                    title={language === 'BN' ? 'বাতিল করুন' : 'Decline'}
                    className="p-2 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 disabled:opacity-50 text-slate-600 dark:text-slate-300 rounded-lg transition"
                  >
                    <X className="w-4 h-4" />
                  </button>
                </UserRow>
            ...
            sentRequests.map(r => {
              ...
              return (
                <UserRow key={r.id} user={u}>
                  ...
                  <button
                    onClick={() => runAction(r.id, () => cancelFriendRequest(r.id))}
                    disabled={pendingActionId === r.id}
                    className="px-3 py-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 disabled:opacity-50 text-slate-600 dark:text-slate-300 font-semibold text-xs rounded-lg transition"
                  >
          ...
          {friends.length === 0 ? (
            ...
            friends.map(u => (
              <UserRow key={u.id} user={u}>
                ...
                <button
                  onClick={() => runAction(u.id, () => removeFriend(u.id))}
                  disabled={pendingActionId === u.id}
                  title={language === 'BN' ? 'আনফ্রেন্ড' : 'Unfriend'}
                  className="p-2 bg-slate-100 dark:bg-slate-800 hover:bg-red-50 dark:hover:bg-red-950/40 disabled:opacity-50 text-slate-500 hover:text-red-600 rounded-lg transition"
                >
