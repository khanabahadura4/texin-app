...
export const ProfileView: React.FC<ProfileViewProps> = ({ userId, onBack, onOpenMessage }) => {
  ...
  const runFriendAction = async (action: () => Promise<void>) => {
    ...
    try {
      ...
      setFriendActionError(
        err?.message ||
          (language === 'BN' ? 'অ্যাকশনটি সম্পন্ন করা যায়নি। আবার চেষ্টা করুন।' : 'This action failed. Please try again.')
      );
    } finally {
  ...
  const [eduUniversity, setEduUniversity] = useState(user.education?.university || '');
  const [eduDepartment, setEduDepartment] = useState(user.education?.department || '');
  const [eduBatchNumber, setEduBatchNumber] = useState(user.education?.batchNumber || '');
  const [eduPassingYear, setEduPassingYear] = useState(user.education?.passingYear || '');
  ...
  const handleSaveProfile = (e: React.FormEvent) => {
    ...
    const updatedUser: UserProfile = {
      ...
      education: {
        ...
        universityFullName: UNIVERSITIES_LIST.find(u => u.id === eduUniversity)?.fullName || user.education?.universityFullName || eduUniversity,
        department: eduDepartment,
        batchNumber: eduBatchNumber,
        passingYear: eduPassingYear
      },
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-md overflow-hidden">
              ...
              {isFriend ? (
                <>
                  ...
                  <button
                    ...
                    onClick={() => runFriendAction(() => removeFriend(user.id))}
                ...
                <button
                  disabled={isFriendActionPending}
                  onClick={() => runFriendAction(() => cancelFriendRequest(relevantRequest.id))}
                ...
                <button
                  disabled={isFriendActionPending}
                  onClick={() => runFriendAction(() => acceptFriendRequest(relevantRequest.id))}
                  className="px-3 py-1.5 bg-[#005244] hover:bg-[#002e26] text-white font-bold text-xs rounded-xl shadow-lg transition flex items-center space-x-1.5"
                >
                ...
                <button
                  disabled={isFriendActionPending}
                  onClick={() => runFriendAction(() => sendFriendRequest(user.id))}
                  className="px-3 py-1.5 bg-[#005244] hover:bg-[#002e26] text-white font-bold text-xs rounded-xl shadow-lg transition flex items-center space-x-1.5"
                >
        ...
        {friendActionError && (
          <div className="mx-4 sm:mx-6 mt-3 flex items-start gap-2 bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-900 text-red-700 dark:text-red-400 text-xs rounded-xl p-3">
            <span className="flex-1">{friendActionError}</span>
            <button onClick={() => setFriendActionError('')} className="font-bold shrink-0">×</button>
          </div>
        )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="bg-white dark:bg-slate-900 rounded-2xl max-w-2xl w-full p-6 shadow-2xl border border-slate-200 dark:border-slate-800 space-y-4 my-8">
            ...
            <form onSubmit={handleSaveProfile} className="space-y-4 max-h-[70vh] overflow-y-auto p-1">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold mb-1 dark:text-slate-300">Birth Date</label>
                  <input
                    type="date"
                    value={birthDate}
                ...
                <div>
                  <label className="block text-xs font-semibold mb-1 dark:text-slate-300">Gender</label>
                  <select
                    value={gender}
              ...
              <div className="space-y-2 pt-2 border-t dark:border-slate-800">
                <span className="text-xs font-bold uppercase dark:text-slate-300 flex items-center gap-1.5">
                  <GraduationCap className="w-3.5 h-3.5 text-brand-600" />
                  {language === 'BN' ? 'শিক্ষা' : 'Education'}
                </span>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-semibold mb-1 dark:text-slate-300">University</label>
                    <select
                      value={eduUniversity}
                  ...
                  <div>
                    <label className="block text-xs font-semibold mb-1 dark:text-slate-300">Department</label>
                    <input
                      type="text"
                      value={eduDepartment}
                      onChange={e => setEduDepartment(e.target.value)}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-semibold mb-1 dark:text-slate-300">Batch</label>
                    <input
                      type="text"
                      value={eduBatchNumber}
                      onChange={e => setEduBatchNumber(e.target.value)}
                      placeholder="e.g. 41st Batch"
                      className="w-full px-3 py-2 text-xs border rounded-lg dark:bg-slate-800 dark:border-slate-700 dark:text-white"
                    />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold mb-1 dark:text-slate-300">Passing Year</label>
                    <input
                      type="text"
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs font-semibold mb-1 dark:text-slate-300">Joining Year</label>
                  <input
                    type="text"
                    value={joiningYear}
              ...
              <div>
                <label className="block text-xs font-semibold mb-1 dark:text-slate-300">
                  {language === 'BN' ? 'দক্ষতা (কমা দিয়ে আলাদা করুন)' : 'Skills (comma separated)'}
                </label>
                <input
                  type="text"
                  value={skillsText}
                  onChange={e => setSkillsText(e.target.value)}
                  placeholder="e.g. Yarn Testing, Wet Processing, Buyer Sourcing"
                  className="w-full px-3 py-2 text-xs border rounded-lg dark:bg-slate-800 dark:border-slate-700 dark:text-white"
                />
              </div>
